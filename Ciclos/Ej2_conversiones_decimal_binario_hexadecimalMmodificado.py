...
def suma_de_binarios():
    primer_numero_binario = input("Ingrese un número binario: ")
    lista_digitos_primer_binario = [int(un_digito) for un_digito in primer_numero_binario]
    segundo_numero_binario = input("Ingrese un número binario: ")
    lista_digitos_segundo_binario = [int(un_digito) for un_digito in segundo_numero_binario]

    # Para convertir de binario a decimal el primer número
    numero_decimal_1 = 0
    exponente = 0
    digito_invertido = -1

    for i in lista_digitos_primer_binario:
        numero_decimal_1 += (2 ** exponente) * lista_digitos_primer_binario[digito_invertido]
        exponente += 1
        digito_invertido -= 1


    #Para convertir de binario a decimal el segundo número
    numero_decimal_2 = 0
    exponente = 0
    digito_invertido = -1
    for i in lista_digitos_segundo_binario:
        numero_decimal_2 += (2 ** exponente) * lista_digitos_segundo_binario[digito_invertido]
        exponente += 1
        digito_invertido -= 1

    numero_decimal = numero_decimal_1+numero_decimal_2
    ...
    while flag_decimal_binario == 0:
        ...
        if numero_decimal_auxiliar == 0 or numero_decimal_auxiliar == 1:  # Sí el resultado de la división entera del numero decimal entre dos es 1 o 0, significa que debe terminar el ciclo
            lista_binario.append(
                numero_decimal_auxiliar)  # Añade a la lista el nuevo dígito, también este caso significa que es el último caso
            flag_decimal_binario = 1  # Cambia la bandera y rompe el ciclo
    print(f"la suma en binario es: 0b", end="")
    # Invertir lista
...
def decimal_a_binario_hexadecimal():  # Función para convertir de decimal al binario
    ...
    while flag_decimal_binario == 0:
        ...
        if numero_decimal_auxiliar == 0 or numero_decimal_auxiliar == 1:  # Sí el resultado de la división entera del numero decimal entre dos es 1 o 0, significa que debe terminar el ciclo
            lista_binario.append(
                numero_decimal_auxiliar)  # Añade a la lista el nuevo dígito, también este caso significa que es el último caso
            flag_decimal_binario = 1  # Cambia la bandera y rompe el ciclo
    print(f"{numero_decimal} en binario es 0b", end="")
    # Invertir lista
    for i in range(1, len(lista_binario) + 1):
        print(f"{lista_binario[i * -1]}",
              end="")  # imprime la lista desde -1 hasta -n, donde -n es el número total de elementos de la lista, recordando que el índice -1 refiere al último elemento de la lista
    ...
    while flag_decimal_hexadecimal == 0:
        ...
        if digito_hexadecimal >= 10 and digito_hexadecimal <= 15:  # Pero si el dígito hexadecimal se encuentra entre el rango de 10 a 15, entonces se reemplaza por una letra
            digito_hexadecimal = lista_letras_hexadecimal[
                digito_hexadecimal - 10]  # Según la letra que le corresponda de la lista_letas_hexadecimal
    ...
    print(f"{numero_decimal} en hexadecimal es : Ox", end = "")
...
def binario_a_decimal_hexadecimal():
    ...
    for i in lista_digitos_binario_auxiliar:
        ...
        exponente += 1
        digito_invertido -= 1
    print(f"0b{numero_binario} en decimal es:")
    ...
    while flag_decimal_hexadecimal == 0:
        ...
        if numero_decimal_auxiliar == 0 or numero_decimal_auxiliar == 1:
            ...
            flag_decimal_hexadecimal = 1
        print(f"{numero_binario} en hexadecimal es: 0x",end="")
        for i in range(1, len(lista_hexadecimal) + 1):
            print(f"{lista_hexadecimal[i * -1]}", end="")
...
def hexadecimal_a_decimal_binario():
    ...
    numero_decimal = 0#estaserálasumatoriadelosdígitosconun1
    exponente = 0
    digito_invertido = -1
    ...
    print(f"{numero_hexadecimal} en binario es: 0b", end="")
    # Invertir lista
    for i in range(1, len(lista_binario) + 1):
        print(f"{lista_binario[i * -1]}",
              end="")  # imprime la lista desde -1 hasta -n, donde -n es el número total de elementos de la lista, recordando que el índice -1 refiere al último elemento de la lista
